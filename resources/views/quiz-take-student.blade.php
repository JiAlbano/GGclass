...
<!--     <div class="question-numbers">
    ...
    <script>
            ...
            if (questions[number - 1].type === 'multipleChoice') {
                ...
                options.map((item, index) => {
                    if (answerExist !== undefined || answerExist != null) {
                        if (answerExist.answer === item) {
                            optionHtml +=
                                `<button class="option-btn active" id="option-btn-${parseInt(index)+1}" onclick="selectOption('${parseInt(index)+1}')">${item}</button>`;
                            $("#answer-holder").val(item)
                        } else {
                            optionHtml +=
                                `<button class="option-btn" id="option-btn-${parseInt(index)+1}" onclick="selectOption('${parseInt(index)+1}')">${item}</button>`;
                        }
                    } else {
                        optionHtml +=
                            `<button class="option-btn" id="option-btn-${parseInt(index)+1}" onclick="selectOption('${parseInt(index)+1}')">${item}</button>`;
                ...
                if (answerExist !== undefined || answerExist != null) {
                    if (answerExist.answer === 'true') {
                        trueFalseAnswer +=
                            `<button class="option-btn active" id="option-btn-true" onclick="selectOption(true)">True</button>`;
                    ...
                    if (answerExist.answer === 'false') {
                        trueFalseAnswer +=
                            `<button class="option-btn active" id="option-btn-false" onclick="selectOption(false)">False</button>`;
        ...
        function recordAnswer() {
            const is_correct = $("#answer-holder").val().toLowerCase() === questions[currentQuestion - 1].correct_answer
                .toLowerCase() ? 1 : 0;
    ...
    <!-- Modal -->
    <div class="modal" id="scoreModal" tabindex="-1" role="dialog" aria-labelledby="scoreModalLabel"
        aria-hidden="true">
